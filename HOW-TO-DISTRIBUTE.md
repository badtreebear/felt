...
### Local LLM (Ollama) — set OLLAMA_ORIGINS to the app, not `*`
The desktop app's webview makes cross-origin requests to Ollama, so Ollama needs to
allow the app's origin via the `OLLAMA_ORIGINS` environment variable. **Don't use `*`** —
that lets any website you visit in a browser reach your local Ollama (run inference,
pull/delete models). Scope it to just the app instead:

```
OLLAMA_ORIGINS=http://tauri.localhost,https://tauri.localhost,tauri://localhost
```

(`http(s)://tauri.localhost` covers the Windows build; `tauri://localhost` covers macOS.)
Set it as a system/user environment variable, then restart Ollama. Also keep
`OLLAMA_HOST` at its default `127.0.0.1` — never `0.0.0.0`, which exposes the
unauthenticated API to your whole network.
