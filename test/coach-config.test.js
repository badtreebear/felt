...
describe("coach config", () => {
  ...
  it("saves and reloads normalized runtime settings", () => {
    ...
    // The key is no longer persisted to localStorage (it lives in the OS keychain
    // / a separate browser entry via coach/key-store.js), so a reload returns
    // everything except the key, and the raw storage never contains it.
    expect(loadCoachConfig({ storage })).toEqual({ ...saved, apiKey: "" });
